          ...
          <table className="w-full text-xs sm:text-sm min-w-[450px]">
            <thead>
              <tr className="text-text-secondary border-b border-border">
                ...
                <th className="text-right py-2 sm:py-3 px-2 sm:px-4">Last Seen</th>
                <th className="text-right py-2 sm:py-3 px-2 sm:px-4">Function</th>
              </tr>
            </thead>
            <tbody>
              {data.map((err, i) => (
                <tr
                  key={i}
                  className="border-b border-border/50 hover:bg-bg-input/50 transition"
                >
                  <td className="py-2 sm:py-3 px-2 sm:px-4 font-mono text-error max-w-[150px] sm:max-w-[250px] lg:max-w-none">
                    <div className="overflow-x-auto whitespace-nowrap scrollbar-none" title={err.error}>
                      {err.error}
                    </div>
                  </td>
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-right font-medium align-top">{err.count.toLocaleString()}</td>
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-right text-text-secondary align-top">{err.lastSeen}</td>
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-right font-mono max-w-[150px] align-top">
